...
local function find_ns_cat_site_keys(partner_id, ns, category)
    ...
    for j=1,#available_sites do
        nscat_keys[#nscat_keys+1] = 'simple/ns:' .. ns .. ':cat:' .. category .. ':site:' .. available_sites[j]
    end
    return nscat_keys
end
...
local function bitop_and(key, vals)
    return redis.call('SINTERSTORE', key, unpack(vals))
end

local function bitop_or(key, vals)
    return redis.call('SUNIONSTORE', key, unpack(vals))
end
